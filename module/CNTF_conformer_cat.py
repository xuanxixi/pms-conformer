import torch
from wenet.transformer.encoder_cat import CNTF_ConformerEncoder
...
class Conformer(torch.nn.Module):
    def __init__(self, n_mels=80, num_blocks=6, output_size=256, embedding_dim=192, input_layer="conv2d2", 
        ...
        self.conformer = CNTF_ConformerEncoder(input_size=n_mels, num_blocks=num_blocks,
...
def CNTF_conformer_cat(n_mels=80, num_blocks=6, output_size=256,
    ...
    model = Conformer(n_mels=n_mels, num_blocks=num_blocks, output_size=output_size, 
            embedding_dim=embedding_dim, input_layer="CNTF", pos_enc_layer_type=pos_enc_layer_type)
    return model

model= CNTF_conformer_cat()
print(model)
